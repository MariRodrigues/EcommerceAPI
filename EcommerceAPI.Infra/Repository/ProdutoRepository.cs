...
namespace EcommerceAPI.Infra.Repository
{
    public class ProdutoRepository : IProdutoRepository
    {
        ...
        public ProdutoRepository(AppDbContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public Produto GetById(int id)
        {
            ...
            using var connection = new MySqlConnection(_configuration.GetConnectionString("CategoriaConnection"));
            connection.Open();

            var result = connection.QuerySingleOrDefault<Produto>(sql, new { id });
        ...
        public IEnumerable<Produto> GetAllFilter(FiltrosProduto filtros)
        {
            ...
            var result = connection.Query<Produto, Categoria, CentroDistribuicao, Produto>
                (sql, (produto, categoria, centro) => {
                    produto.CategoriaId = categoria.Id;
                    produto.CentroDistribuicaoId = centro.Id;
                    return produto; 
                }, queryArgs, splitOn: "CategoriaId, CentroDistribuicaoId")
                .Skip((filtros.Pagina - 1) * filtros.ItensPagina).Take(filtros.ItensPagina);
